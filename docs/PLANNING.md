...
Design source of truth: `Everly Mobile Auth.dc.html` and `Everly Mobile.dc.html` in `../Everly bucket list app/` (one level above this repo, same location referenced by `everly`'s own design docs) — both are iPhone-frame, mobile-native layouts, not resized desktop. Logo asset: `everly-logo-dark.png` from the same folder.
