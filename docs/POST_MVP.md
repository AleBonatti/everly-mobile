...
**Status**: auth screens (login, register, forgot-password) — **done, 2026-08-24**. Still to do: items list (`app/index.tsx`), item edit (`app/item/[id].tsx`), category screens, settings screen — same components/tokens/pattern now established, should go faster than the auth screens did. Also still open: replacing the Unicode-character icon stand-ins from Group D (⌕, ☰, ⇅, ▦) with real icons — not yet decided how (icon library vs. SVG set).
