...
**Loading-state inconsistency (original finding, 2026-08-21) — resolved for auth screens via `PrimaryButton`**, per above. Still needs applying to `app/item/[id].tsx` (already had a spinner, but not the `opacity-60` disabled-dimming) and `app/category/[id].tsx`/`app/settings.tsx` (still using inline buttons, not yet migrated to `PrimaryButton`) when those screens are retrofitted.
